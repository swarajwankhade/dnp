...
int serverSD,serverUDPSD,clientSD;
int seg1,seg2,seg3;
...
main( int argc, char **argv )
{
	...
	setLoopback(serverUDPSD, loop);
 	...
 	serverUnicastUDPSocketAddress.sin_addr.s_addr  =  htonl (INADDR_ANY);
  	...
  	serverUsd = socket (AF_INET,SOCK_DGRAM,IPPROTO_UDP); 
  	reusePort(serverUsd);
			
	
	if(bind (serverUsd,  (SA *) &serverUnicastUDPSocketAddress, sizeof(serverUnicastUDPSocketAddress)) < 0)
	{
  		...
  		if((unicastChildID = fork()) == 0)
  		{
			...
			ufromlength=sizeof(clientInSocketAddress);
  			...
  			while(1) 
  			{
				...
				ubytes = recvfrom(serverUsd, urecvBuf, BUFSIZE, 0,(SA*)&clientInSocketAddress,&ufromlength);
				...
				{	
					if(strncmp(L,urecvBuf,6) == 0)
					{
						...
						if(count[0].uClientsCount > 0)
						{
							...
							if ((c=sendto(serverUsd, msg, strlen(msg), 0, (SA *) & clientUnicastUDPSocketAddress, sizeof(clientUnicastUDPSocketAddress))) < 0) 
							{
						...
						if(count[0].mClientsCount > 0)
						{
							...
							if ((c=sendto(serverUsd, L, strlen(L), 0, (SA *) & clientUnicastUDPSocketAddress, sizeof(clientUnicastUDPSocketAddress))) < 0) 
							{
								perror("error in UList sendto \n");
								exit(-1);
							}
					...
					else if(strncmp(E,urecvBuf,6)==0)
					{
						...
						if(count[0].uClientsCount > 0)
						{
							...
							if ((c=sendto(serverUsd, E, strlen(E), 0, (SA *) & clientUnicastUDPSocketAddress, sizeof(clientUnicastUDPSocketAddress))) < 0) 
							{
						...
						printf("Updated Client List is \n");
						printLists();
					}
					else
		 			 {
		 		 		char msg[1000] = "";
		 		 		sprintf(msg,"(From Unicast: %s:%d):%s", inet_ntoa(clientInSocketAddress.sin_addr),ntohs(clientInSocketAddress.sin_port),urecvBuf);
		 		 		sendUTOM(msg);
		 			 }
				}
			}	
...
void getMessage(int inSock, int outSock)
{
	...
	for (;;)
	{
		...
		bytes = recvfrom(inSock, recvBuf, BUFSIZE, 0,(SA*)&clientInSocketAddress,&fromlength);
		if (bytes < 0) 
		{
		...
		{	
		 	 char msg[1000] = "";
		 	 sprintf(msg,"(From Multicast: %s:%d):%s", inet_ntoa(clientInSocketAddress.sin_addr),ntohs(clientInSocketAddress.sin_port),recvBuf);
		 	 sendMTOU(msg);		  
		}
	}
}
...
void groupExit(int recvSock, char *group)
{
	...
	printf("process that quit multicast group %s \n", group);
}
