...
void printLists();
void sendData(int clientSD, int falg);
...
int serverSD,serverUDPSD,clientSD,serverUNIUDPSD;
int clientListenerSD,uniClientListenerSD;
...
main( int argc, char **argv )
{
	...
	if ((rc1 = pthread_create(&thread1, NULL, &handleTCPClients, NULL)))
	{
		printf("Send Message Thread creation failed: %d\n", rc1);
	}
	
	if ((rc2 = pthread_create(&thread2, NULL, &handleUnicastMessages, NULL)))
	{
		printf("Get Message Thread creation failed: %d\n", rc2);
	}
	
	if ((rc3 = pthread_create(&thread3, NULL, &handleMultiCastMessages, NULL)))
	{
		printf("Get Message Thread creation failed: %d\n", rc3);
	}
...
void sendData(int clientSD, int flag)
{
